...
/**
 * Abstract Base Class - Contains the core heuristics and Link header generation logic.
 *
...
abstract class Base
{
    public function __construct()
    {
        ...
        // Optimizations for Block Themes (FSE)
        add_filter('should_load_separate_core_block_assets', [$this, 'should_load_separate_core_block_assets']);
        add_filter('styles_inline_size_limit', [$this, 'styles_inline_size_limit']);
    }

    /**
     * Determine if WordPress should load styles only for rendered blocks on the current page.
     */
    public function should_load_separate_core_block_assets($default)
    {
        $value = get_option('hlhg_load_separate_assets', '1');
        $value = filter_var($value, FILTER_VALIDATE_BOOLEAN);

        return apply_filters('hlhg_should_load_separate_core_block_assets', $value);
    }

    /**
     * Determine the stylesheet inlining size limit in bytes.
     */
    public function styles_inline_size_limit($default)
    {
        $value = get_option('hlhg_inline_size_limit', '0');
        $value = intval($value);

        return apply_filters('hlhg_styles_inline_size_limit', $value);
    }
    ...
    public function inspect_and_emit_link_headers()
    {
        ...
        if (is_admin() || wp_is_json_request() || $this->headers_sent()) {
            return;
        }
        ...
        if (!empty($preload_candidates)) {
            ...
            $this->send_link_header(implode(', ', $link_headers));
        }
    }
    ...
    /**
     * Emit Link HTTP response header.
     */
     ...
     */
    protected function resolve_asset_url($src, $ver)
    {
